...
{
    va_list arguments;
    fprintf(stderr, "JIT: serious error: ");
...
{
#ifdef WINUAE_FOR_PREVIOUS
    /* Keep MMU-backed memory blocks interpreted until the ARM64 generated
     * memory-call ABI has been validated against Previous's reduced banks. */
    return true;
#else
...
#ifdef WINUAE_FOR_PREVIOUS
static uae_u32 (*previous_saved_next_iword)(void);
static uae_u32 (*previous_saved_next_ilong)(void);
static uae_u32 (*previous_saved_cp_next_iword)(void);
static uae_u32 (*previous_saved_cp_next_ilong)(void);
static bool previous_fallback_fetches_active;

extern "C" void previous_jit_restore_fallback_fetches(void)
{
    if (!previous_fallback_fetches_active)
        return;
    x_next_iword = previous_saved_next_iword;
    x_next_ilong = previous_saved_next_ilong;
    x_cp_next_iword = previous_saved_cp_next_iword;
    x_cp_next_ilong = previous_saved_cp_next_ilong;
    previous_fallback_fetches_active = false;
}

extern "C" uae_u32 previous_jit_execute_fallback(uae_u32 opcode)
{
    /* Generated blocks can reach this wrapper after executing several native
     * instructions.  Capture the now-materialized flags at the actual faulting
     * instruction boundary, as the 68040 restart path does in the interpreter. */
    previous_jit_checkpoint_exception_state();
    previous_saved_next_iword = x_next_iword;
    previous_saved_next_ilong = x_next_ilong;
    previous_saved_cp_next_iword = x_cp_next_iword;
    previous_saved_cp_next_ilong = x_cp_next_ilong;
    previous_fallback_fetches_active = true;

    x_next_iword = previous_jit_next_iword_mmu040;
    x_next_ilong = previous_jit_next_ilong_mmu040;
    x_cp_next_iword = previous_jit_next_iword_mmu040;
    x_cp_next_ilong = previous_jit_next_ilong_mmu040;

    uae_u32 cycles = nfcpufunctbl[opcode](opcode);
...
#ifdef WINUAE_FOR_PREVIOUS
/*
 * Previous's 68040 MMU raises guest faults with setjmp/longjmp.  A fault while
 * compile_block() is emitting code bypasses jit_end_write_window(), leaving
 * the current macOS thread in MAP_JIT write mode.  The next dispatch into the
 * cache then fails with EXC_BAD_ACCESS even though vmmap reports an RWX
 * mapping.  Dispatcher and exception paths use this to restore a known state.
 */
extern "C" void jit_force_execute_mode(void)
{
	jit_write_window_depth = 0;
#if defined(__APPLE__) && defined(CPU_AARCH64)
	uae_vm_jit_write_protect(true);
...
{
#if defined(UAE) && !defined(WINUAE_FOR_HATARI)
    ...
    return 0;
#elif defined(WINUAE_FOR_PREVIOUS)
    (void)addr;
    return 0;
#else
...
#if defined(CPU_AARCH64)
	/* check_prefs_changed_comp() can request the cache before build_comp()
	 * has created the dispatcher stubs.  Branch reachability cannot be judged
	 * against a null popallspace, and repeatedly halving the cache here leaves
	 * the later compiler initialization with a tiny, separately allocated
	 * cache.  build_comp() calls alloc_cache() again after create_popalls(),
	 * which can then use the contiguous ARM64 stub+cache allocation. */
	if (!popallspace) {
		jit_log("ARM64: deferring JIT cache allocation until dispatcher stubs exist");
		return;
	}
...
static void calc_checksum(blockinfo* bi, uae_u32* c1, uae_u32* c2)
{
    ...
    while (csi) {
        ...
        if (len >= 0 && len <= MAX_CHECKSUM_LEN) {
            while (len > 0) {
#ifdef WINUAE_FOR_PREVIOUS
                uae_u32 value = jit_fetch_long((uaecptr)(uintptr)pos);
                k1 += value;
                k2 ^= value;
#else
                k1 += *pos;
...
#ifdef WINUAE_FOR_PREVIOUS
#define DO_GET_OPCODE(a) (jit_fetch_word((uaecptr)(uintptr)(a)))
#else
 ...
 * sentinel, so the suite stays interpreted.
 *
...
{
#ifdef WINUAE_FOR_PREVIOUS
    /* jit_fetch_word() already returns the guest opcode in host byte order.
     * get_opcode_cft_map() is only for opcodes loaded directly from the
     * little-endian host mapping; applying it here classified byte-swapped
     * instructions and failed to quarantine RTE/SR-changing blocks. */
    switch (table68k[op].mnemo) {
#else
...
void compile_block(cpu_history* pc_hist, int blocklen, int totcycles)
{
    if (cache_enabled && compiled_code && currprefs.cpu_model >= 68020) {
		...
		int extra_len=0;
#if defined(CPU_AARCH64)
        const bool indirect_rtarea_block = false;
        /* NeXT firmware probes devices by deliberately taking 68040 bus
         * faults.  Keep ROM blocks on the proven MMU interpreter path; start
         * native compilation only after the guest has entered RAM. */
#ifdef WINUAE_FOR_PREVIOUS
        bool unsafe_special_mem_block = indirect_rtarea_block || trace_in_rom;
#else
...
#if defined(WINUAE_FOR_PREVIOUS) && defined(NOFLAGS_SUPPORT_GENCOMP) && !defined(NOFLAGS_SUPPORT_GENCPU)
                /* Previous runs the JIT together with the 68040 MMU.  Its
                 * generated no-flags CPU table contains full-flags handlers
                 * (NOFLAGS_SUPPORT_GENCPU is intentionally disabled), and is
                 * populated from uaegetjitcputbl's MMU-aware table.  Use it
                 * for every untranslated opcode: the global cpufunctbl is the
                 * direct/no-MMU JIT table and cannot safely service memory
                 * fallbacks such as BFEXTU. */
                cputbl = nfcpufunctbl;
#endif
...
#ifdef WINUAE_FOR_PREVIOUS
                    /* All untranslated instructions need scalar-PC MMU fetches
                     * for extension words.  Calling the table entry directly
                     * bypassed the wrapper used by optlevel-0 blocks. */
                    compemu_raw_call((uintptr)previous_jit_execute_fallback);
#else
