...
void esp_start_command(uint8_t cmd) {
    ...
    if ((cmd&CMD_TYP_MASK)!=CMD_TYP_MSC) {
        if ((esp_state==TARGET && !(cmd&CMD_TYP_TGT)) ||
            ...
            esp_command_clear();
            intstatus |= INTR_ILL;
            CycInt_AddTimeInterrupt(ESP_DELAY, 20, INTERRUPT_ESP);
            return;
        }
    }
...
void esp_bus_reset(void) {
    ...
    if (!(configuration & CFG1_RESREPT)) {
        ...
        CycInt_AddCycleTimeInterrupt(ConfigureParams.System.nMachineType == NEXT_CUBE030 ? 50 : 335, 0, INTERRUPT_ESP);
    } else {
...
void esp_select(bool atn) {
    ...
    if (timeout) {
        ...
        CycInt_AddTimeInterrupt(seltout, 0, INTERRUPT_ESP);
        return;
    }
...
void esp_transfer_info(void) {
    if (mode_dma) {
        ...
        CycInt_AddTimeInterrupt(SCSIdisk_Time(), 100, INTERRUPT_ESP_IO);
    } else {
        Log_Printf(LOG_ESPCMD_LEVEL, "[ESP] start PIO transfer");
        switch (SCSIbus.phase) {
            case PHASE_DI:
                esp_fifo_write(SCSIdisk_Send_Data());
                CycInt_AddCyclesInterrupt(20, INTERRUPT_ESP);
                break;
            case PHASE_MI:
                CycInt_AddCyclesInterrupt(20, INTERRUPT_ESP);
                break;
...
void ESP_IO_Handler(void) {
    ...
    CycInt_UpdateTimeInterrupt(100, 0, INTERRUPT_ESP_IO);
}
